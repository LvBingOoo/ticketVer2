...
public class HomeBuyGridType1Adapter extends BaseMultiAdapter<HomeBuyEntry> implements View.OnClickListener {

    private  int mScreenWidth;
    private DensityUtil densityUtil;
    onItemClickListenter mOnItemClick;
    private int mPrarentPos = 0;
    public HomeBuyGridType1Adapter(Context context, int screenWidth , onItemClickListenter click, int pos) {
        super(context);
        addItemType(Const.TYPE_01, R.layout.item_bugygride_type1);
    ...
    public void onBindItemHolder(SuperViewHolder holder, final int position) {
        ...
        switch (item.getItemType()){
            case Const.TYPE_01:
                View rootView = holder.getView(R.id.gride_type1root);
                rootView.setTag(position);
                rootView.setOnClickListener(this);

                TextView grid_name = holder.getView(R.id.grid_name);
                ImageView item_buytype1_iv = holder.getView(R.id.item_buytype1_iv);
                Glide.with(mContext).load(item.icon).fitCenter().dontAnimate().into(item_buytype1_iv);
